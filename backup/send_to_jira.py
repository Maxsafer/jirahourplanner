    ...
    for activity in activities:
        ...
        for wl in existing:
            ...
            if wl_sec == target_seconds and wl_comm == comment and wl_auth == email:
                logs.append(f"⚠️ Already logged {int(hours)}h to {issue_key} with comment: “{comment}”")
                dup = True
                break
        if dup:
            continue
        ...
        try:
            jira.add_worklog(
                issue=issue,
                timeSpent=f"{int(hours)}h",
                comment=comment
            )
            logs.append(f"✅ Logged {int(hours)}h to {issue_key}: “{comment}”")
        except Exception as e:
